---
title: Villager Shop
description: Player-run villager shops with stock, payment storage and an in-game trade editor.
---

Let players set up their own villager-run shops. An owner spawns a shop villager, stocks goods, builds trades and prices in an in-game editor, and collects payments from a storage screen — a self-service market district with no command wrangling.

## Requirements

- Minecraft **26.3**, **26.2** or **26.1.2**, **Fabric** with Fabric API
- Server-side friendly — menus and messages are drawn per player from the server, so vanilla clients work too.

## Download

- [CurseForge](https://www.curseforge.com/minecraft/mc-mods/villagerbased-playershops)
- [Modrinth](https://modrinth.com/mod/villagerbased-playershops)

## Features

- **Owner setup menu** — right-click your shop villager to manage it: rename the shop, change the villager's profession and biome variant, rotate it, toggle a glow outline, and remove it.
- **Trade editor** — add trades, set the goods sold and the price items, and manage each trade's stock. Left-click an existing trade to adjust its price with +/- buttons; right-click to remove it. Trades grey out when their stock runs dry.
- **Stock & payments** — a stock container holds what you're selling (sold goods are deducted from it), and a separate collected-payments screen is where you withdraw what customers paid.
- **`/shop` commands** — create shops, list your shops or all shops, rename, and remove (items are returned to you).
- **Fully localized** — every menu, the trade editor, and all command messages are translatable per player. English and **Traditional Chinese (zh_tw)** ship built in.

## Links

- [CurseForge](https://www.curseforge.com/minecraft/mc-mods/villagerbased-playershops)
- [Modrinth](https://modrinth.com/mod/villagerbased-playershops)
- [GitHub — 26.3 and 26.2](https://github.com/Andrewwwwwwwwwwwwwww/villagershop) (26.2 on the `mc26.2` branch)
- [GitHub — 26.1.2](https://github.com/Andrewwwwwwwwwwwwwww/villagershop-mc26.1.2)
